...
TEST_CASE("ShellSort", "[SHELLSORT]") {
  ...
  SECTION("sorting 10 elements - simple with 2Square") {

      std::vector<int> b(V);
      sorting::ShellSort_2Square(b, b.size());
...
TEST_CASE("CountingSort", "[COUNTINGSORT]") {

    SECTION("sorting 10 elements - simple") {

        std::vector<int> e(V);

        sorting::CountingSort(e, 0, e.size()-1);
...
TEST_CASE("RadixSort", "[RADIXSORT]") {

    SECTION("sorting 10 elements - simple") {

        std::vector<int> f(V);

        sorting::RadixSort(f, 0, f.size() - 1);
